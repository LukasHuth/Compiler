...
AST *AST_new_node()
{
    AST ast = { .tag = AST_NODE, .data = { .AST_NODE = { .children = (AST**) calloc(sizeof(AST), 0), .array_size = 0 } } };
    return AST_new(ast);
}
...
void AST_add_child(AST* ast, AST* child)
{
    ...
    if (ast->tag == AST_NODE)
    {
        ...
        ast->data.AST_NODE.children = (AST**) realloc(ast->data.AST_NODE.children, sizeof(AST*) * (ast->data.AST_NODE.array_size));
...
void AST_add_argument(AST* ast, AST* argument)
{
    if(ast->tag == AST_FUNCTION)
    {
        ...
        ast->data.AST_FUNCTION.arguments = (AST**) realloc(ast->data.AST_FUNCTION.arguments, sizeof(AST*) * (ast->data.AST_FUNCTION.array_size));
    ...
    if(ast->tag == AST_CALL)
    {
        ...
        ast->data.AST_CALL.arguments = (AST**) realloc(ast->data.AST_CALL.arguments, sizeof(AST*) * (ast->data.AST_CALL.array_size));
...
AST *AST_new_break()
{
    AST ast = { .tag = AST_BREAK, .data = { NULL } };
    return AST_new(ast);
}
AST *AST_new_noop()
{
    AST ast = { .tag = AST_NOOP, .data = { NULL } };
    return AST_new(ast);
}
AST *AST_new_continue()
{
    AST ast = { .tag = AST_CONTINUE, .data = { NULL } };
    return AST_new(ast);
}

...
void variable_array_add_variable(VARIABLE_ARRAY *array, VARIABLE *variable)
{
    ...
    array->size++;
    array->variables = (VARIABLE**) realloc(array->variables, sizeof(VARIABLE*) * array->size);
...
{
    VARIABLE_ARRAY *array = (VARIABLE_ARRAY*) calloc(1, sizeof(VARIABLE_ARRAY));
    array->size = 0;
...
VARIABLE *create_variable(AST *type, char *name)
{
    ...
    variable->type = type;
    variable->name = (char*) calloc(strlen(name) + 1, sizeof(char));
...
{
    FUNC_ARRAY *array = (FUNC_ARRAY*) calloc(1, sizeof(FUNC_ARRAY));
    array->size = 0;
...
FUNC *create_function(char *name)
{
    ...
    FUNC *function = (FUNC*) calloc(1, sizeof(FUNC));
    function->name = (char*) calloc(strlen(name) + 1, sizeof(char));
...
void FUNCTION_add_attribute(FUNC *function, AST *attribute)
{
    ...
    {
        function->attributes = (AST**) calloc(1, sizeof(AST*));
        function->size = 1;
        function->attributes[0] = attribute;
        return;
    }
    function->size++;
    function->attributes = (AST**) realloc(function->attributes, sizeof(AST*) * function->size);
...
void FUNCTION_ARRAY_add_function(FUNC_ARRAY *array, FUNC *function)
{
    ...
    {
        array->functions = (FUNC**) calloc(1, sizeof(FUNC*));
        array->size = 1;
        array->functions[0] = function;
        return;
    }
    array->size++;
    array->functions = (FUNC**) realloc(array->functions, sizeof(FUNC*) * array->size);

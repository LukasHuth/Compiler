...
{
    *_new = (char*) realloc(*_new, strlen(orig) + strlen(format) + 1);
    sprintf(*_new, format, orig);
}
...
LLVMValueRef generate_function_call(AST *ast, InternalInfo *info)
{
    ...
    LLVMValueRef* args = (LLVMValueRef*) calloc(ast->data.AST_CALL.array_size, sizeof(LLVMValueRef));
...
LLVMValueRef generate_expression(AST *ast, InternalInfo *info)
{
    ...
    switch (ast->tag)
    {
        ...
        case AST_VARIABLE:
            ...
            {
                return LLVMGetParam(info->function, ast->data.AST_VARIABLE.arg_index);
            }
            sprintf(temp_name, "_%ld", info->temp_count++);
            temp_name = (char*) realloc(temp_name, sizeof(char) * (strlen(temp_name) + 1));
            value = NULL;
